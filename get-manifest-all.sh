...
mkdir -p /tmp/platform1c
    ...
    curl --fail -b /tmp/cookies.txt -o /tmp/platform1c/${VER}.client64.tar.gz -L "$CLIENTLINK64"

    echo "Закачиваем серверную часть версии $VER для архитектуры x86_64"
    curl --fail -b /tmp/cookies.txt -o /tmp/platform1c/${VER}.server64.tar.gz -L "$SERVERLINK64"
#fi

#if [ ${OSBIT} == '32' ]; then

    echo "Закачиваем клиентскую часть версии $VER для архитектуры x86"
    curl --fail -b /tmp/cookies.txt -o /tmp/platform1c/${VER}.client32.tar.gz -L "$CLIENTLINK32"

    echo "Закачиваем серверную часть версии $VER для архитектуры x86"
    curl --fail -b /tmp/cookies.txt -o /tmp/platform1c/${VER}.server32.tar.gz -L "$SERVERLINK32"
...
echo "Преобразовываем deb пакеты версии $VER в tar.gz"
